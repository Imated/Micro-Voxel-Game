...
pub struct WorldRenderer {
    ...
    chunk_grid: [[[Chunk; WORLD_SIZE.x as usize]; WORLD_SIZE.y as usize]; WORLD_SIZE.z as usize],
    ...
    chunks: TypedArrayBuffer<
        [[[Chunk; WORLD_SIZE.x as usize]; WORLD_SIZE.y as usize]; WORLD_SIZE.z as usize],
    >,
...
impl WorldRenderer {
    pub fn new(context: &RenderContext) -> Self {
        let chunk_grid = [[[Chunk::new_from_empty(); WORLD_SIZE.x as usize]; WORLD_SIZE.y as usize];
    ...
    pub fn load_chunk(&mut self, coords: ChunkPos) {
        let offsetted_coords = coords.0 + WORLD_SIZE_HALF;
        let index = (offsetted_coords.rem_euclid(WORLD_SIZE)).as_usizevec3();
